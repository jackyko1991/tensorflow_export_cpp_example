...
# Global constants describing the CIFAR-10 ckpt set.
...
def inference(images):
  ...
  '''
  images = tf.reshape(images, shape=[-1, 32, 32, 3], name="input_node")

  # Store layers weight & bias
  _weights = {
    'wc1': tf.Variable(tf.random_normal([5, 5, 3, out_conv_1], stddev=1e-3)),  # 5x5 conv, 1 input, 64 outputs
  ...
  with tf.name_scope('Dense2'):
    ...
    '''
    TODO #2 : Give special attention to the name parameter, we need to define which node is output node in our C++
    code
    '''
    logits = tf.add(tf.matmul(dense2, _weights['out']), _biases['out'], name="output_node")
